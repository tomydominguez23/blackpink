(function () {
  ...
  function render() {
    ...
    pay.addEventListener("click", function () {
      ...
      if (!window.BlackpinkWebpayCheckout || !Co) {
        window.alert("Recargá la página: faltan scripts de pago.");
        return;
      }

      var customerData = Co.readForm(root);
      var errs = Co.validate(customerData);
      if (errs.length) {
        if (payErr) {
      ...
      if (customerData.saveInfo) Co.save(customerData);
      else Co.save({ saveInfo: false });

      var stPay = window.BlackpinkCart.load();
      var UUID_RE = window.BlackpinkWebpayCheckout.UUID_RE;
