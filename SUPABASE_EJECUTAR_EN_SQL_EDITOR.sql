-- =============================================================================
-- OBLIGATORIO: pegar y ejecutar TODO este archivo en Supabase → SQL Editor
-- Sin esto, productos con stock 0 no se ven y Agotado por variante no funciona bien.
-- =============================================================================
...
begin
  if not exists (
    ...
    return jsonb_build_object('ok', false, 'error', 'order_not_pending_or_missing');
  end if;
  ...
  loop
    ...
    if target_id is null then
      return jsonb_build_object('ok', false, 'error', 'product_missing', 'product_id', r.product_id);
    end if;
    ...
    if not found then
      return jsonb_build_object(
        ...
        'product_id', r.product_id, 'resolved_stock_owner', target_id
      );
    end if;
  end loop;
